-- Add delivery_method and shipping_address columns to ipf_orders
ALTER TABLE public.ipf_orders 
ADD COLUMN IF NOT EXISTS delivery_method text NOT NULL DEFAULT 'pickup',
ADD COLUMN IF NOT EXISTS shipping_address jsonb;

-- Add a check constraint to validate delivery_method values
ALTER TABLE public.ipf_orders 
ADD CONSTRAINT ipf_orders_delivery_method_check 
CHECK (delivery_method IN ('pickup', 'shipping'));

-- Update the ipf_create_order_with_items function to handle shipping orders
...
BEGIN
  ...
  -- Extract and validate delivery_method (default to 'pickup')
  v_delivery_method := coalesce(trim(payload->>'delivery_method'), 'pickup');
  IF v_delivery_method NOT IN ('pickup', 'shipping') THEN
    RAISE EXCEPTION 'delivery_method must be either pickup or shipping';
  END IF;

  -- Extract and validate shipping_address if delivery_method is 'shipping'
  IF v_delivery_method = 'shipping' THEN
    v_shipping_address := payload->'shipping_address';
    IF v_shipping_address IS NULL OR jsonb_typeof(v_shipping_address) <> 'object' THEN
      RAISE EXCEPTION 'shipping_address is required for shipping orders';
    END IF;
    -- Validate required address fields
    IF v_shipping_address->>'street' IS NULL OR trim(v_shipping_address->>'street') = '' THEN
      RAISE EXCEPTION 'Street address is required';
    END IF;
    IF v_shipping_address->>'city' IS NULL OR trim(v_shipping_address->>'city') = '' THEN
      RAISE EXCEPTION 'City is required';
    END IF;
    IF v_shipping_address->>'state' IS NULL OR trim(v_shipping_address->>'state') = '' THEN
      RAISE EXCEPTION 'State is required';
    END IF;
    IF v_shipping_address->>'zipCode' IS NULL OR trim(v_shipping_address->>'zipCode') = '' THEN
  ...
  INSERT INTO ipf_orders (
    ...
    v_preferred_time,
    v_notes,
    CASE WHEN v_delivery_method = 'shipping' THEN 'pending_confirmation' ELSE 'new' END,
    'pay_at_pickup',
    'unpaid',
    v_delivery_method,
    v_shipping_address
